...
#### Поддержка Landscape
Создана layout-land/activity_second.xml для альбомного режима. В MainActivity.java указан setContentView(R.layout.activity_second) для переопределения стартового экрана. 
...
### 1. Программный подход (через код): Для первой кнопки создан слушатель View.OnClickListener прямо внутри метода onCreate. При клике выводится мой номер по журналу, а чекбокс переводится в активное состояние (setChecked(true)).

### 2. Декларативный подход (через XML): Для второй кнопки в файле разметки прописан атрибут android:onClick="onMyButtonClick". Сам метод реализован в классе MainActivity. При клике чекбокс снимается, текст очищается и вызывается всплывающее системное уведомление Toast.
